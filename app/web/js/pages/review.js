...
UI.register('review', async (slot) => {
    ...
    <div class="wrap" style="padding-top:30px">
      ...
      <div class="stats-row rise">
        <div class="stat-card">
          ...
          <div class="stat-main"><span class="stat-num tabular" id="rvDue">0</span><span class="stat-label">今日待复习</span></div>
        </div>
        <div class="stat-card">
          ...
          <div class="stat-main"><span class="stat-num tabular" id="rvTotal">0</span><span class="stat-label">总题库</span></div>
        </div>
        <div class="stat-card">
          ...
          <div class="stat-main"><span class="stat-num tabular" id="rvMastered">--</span><span class="stat-label">掌握率</span></div>
        </div>
      </div>
      <div class="card rise" style="margin-bottom:var(--gap)">
        <div style="display:flex;gap:12px;margin-bottom:16px">
          <select class="select" id="rvCourseSelect" style="flex:1"><option value="">选择课程</option></select>
          <button class="btn" onclick="UI.rvGenerate()">生成测验</button>
          <button class="btn btn-primary" onclick="UI.rvStart()">开始复习</button>
        </div>
  ...
  try {
    ...
    if (r.success) {
      s.innerHTML = '<option value="">选择课程</option>' + r.courses.map(c => `<option value="${c.id}">${c.name}</option>`).join('');
    }
...
UI.rvGenerate = async function() {
  ...
  if (!cid) return alert('请先选择课程');
  UI.$('#rvStatus').textContent = '正在生成测验题...';
  const r = await eel.api_generate_quizzes(parseInt(cid))();
  if (r.success) {
    UI.$('#rvStatus').textContent = `已生成 ${r.count} 道题`;
...
UI.rvStart = async function() {
  ...
  if (!r.success || !r.reviews || r.reviews.length === 0) {
    UI.$('#rvStatus').textContent = '今天没有待复习的题目';
...
UI.rvRender = function() {
  ...
  let h = `<div class="card card--lift">
    <div style="display:flex;justify-content:space-between;margin-bottom:16px">
      <span style="font-size:12px;color:var(--text-tertiary)">第 ${s.index+1} / ${s.queue.length} 题</span>
  ...
  if (q.options) {
    ...
    h += `<input class="input" id="rvFill" placeholder="输入答案..." onkeydown="if(event.key==='Enter')UI.rvSubmitFill()">
      <button class="btn btn-sm" style="margin-top:12px" onclick="UI.rvSubmitFill()">提交</button>`;
  }
...
UI.rvFeedback = function(correct, q) {
  ...
  fb.innerHTML = `
    <div style="padding:16px;border-radius:12px;background:${correct ? 'var(--green-muted)' : 'var(--red-muted)'}">
      <div style="font-weight:600;color:${correct ? 'var(--green)' : 'var(--red)'};margin-bottom:8px">${correct ? '✅ 正确' : '❌ 错误'}</div>
      <div style="font-size:13px;color:var(--text-secondary);margin-bottom:8px">正确答案: ${UI.esc(q.answer)}</div>
      ${q.explanation ? `<div style="font-size:12px;color:var(--text-tertiary)">${UI.esc(q.explanation)}</div>` : ''}
      <div style="font-size:12px;color:var(--text-tertiary);margin-top:16px">掌握程度?</div>
      <div style="display:flex;gap:8px;margin-top:8px">
        <button class="btn btn-sm" onclick="UI.rvRate(0)">忘了</button>
        <button class="btn btn-sm" onclick="UI.rvRate(3)">模糊</button>
        <button class="btn btn-primary btn-sm" onclick="UI.rvRate(5)">记住了</button>
      </div>
    </div>`;
};

UI.rvRate = async function(q) {
  const quiz = UI.rvState.queue[UI.rvState.index];
  await eel.api_submit_review(quiz.review_id, q)();
  UI.rvState.index++;
  UI.rvRender();
  UI.rvRefreshStats();
};
...
UI.rvSummary = function() {
  ...
  UI.$('#rvArea').innerHTML = `
    <div class="card card--lift" style="text-align:center">
      ...
      <div style="font-size:24px;font-weight:700;font-variant-numeric:tabular-nums">${c} / ${UI.rvState.queue.length} 正确</div>
      <div style="font-size:13px;color:var(--text-tertiary);margin-top:8px">正确率 ${pct}% · 复习计划已更新</div>
    </div>`;
};
